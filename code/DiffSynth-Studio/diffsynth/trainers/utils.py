    ...
    def __init__(
        ...
        if metadata_path is None:
            if raw_dict is not None:
                print("use input condition info...")
                self.data = raw_dict
            else:
                print("No metadata. Trying to generate it.")
                metadata = self.generate_metadata(base_path)
...
def launch_training_task(
    ...
    print(f"directory check: {accelerator.logging_dir} {accelerator.project_dir}")
    ...
    for epoch_id in range(num_epochs):
        ...
        for data in pb:
            with accelerator.accumulate(model):
                ...
                scheduler.step()
                current_index = model_logger.num_steps
                # print(current_index, loss.item())
                #if accelerator.is_local_main_process:
            if accelerator.sync_gradients and accelerator.is_local_main_process:
                # print("logging")
                print(current_index, loss.item())
                # print(model.patch_embedding_extra_concate.__class__)
                accelerator.log(
